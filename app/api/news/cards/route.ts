...
// ─── Supabase client (anon key for public read) ───────────────────────────────
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (batchError) {
      console.error('[news-cards] Batch fetch error:', batchError.message)
    ...
    if (cardsError) {
      console.error('[news-cards] Cards fetch error:', cardsError.message)
    ...
    const formatCard = (card: any) => ({
      ...
      consensus_pct: Math.round(70 + (card.source_count * 5)),
