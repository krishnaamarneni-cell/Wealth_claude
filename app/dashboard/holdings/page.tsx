...
interface CachedHoldingsData {
  ...
  allTimeHigh: number
  timestamp: number
  transactionCount: number // ✅ NEW: Track transaction count
}
...
const getCachedHoldings = (): CachedHoldingsData | null => {
  try {
    ...
    // ✅ NEW: Verify transaction count matches
...
const setCachedHoldings = (data: Omit<CachedHoldingsData, 'transactionCount'>): void => {
  try {
    ...
    const cacheData: CachedHoldingsData = {
      ...data,
      transactionCount: txns.length // ✅ NEW: Store transaction count
    }
...
export default function HoldingsTable() {
  ...
  useEffect(() => {
    ...
    const refreshInterval = setInterval(() => {
      ...
      loadTransactionsAndCalculateHoldings(true) // Silent refresh
    }, 3 * 60 * 60 * 1000)
  ...
  const loadTransactionsAndCalculateHoldings = async (silent = false) => {
    ...
    if (txns.length === 0) {
      ...
      // ✅ NEW: Clear everything when no transactions
      setAllHoldings([])
      setTransactions([])
      setAvailableBrokers([])
      setIsLoading(false)
          ...
          <Card>
            <CardContent className="pt-4 pb-4">
              <div className="flex items-center justify-between mb-2">
                <Button
                  variant={valueView === "portfolio" ? "secondary" : "ghost"}
                  size="sm"
                  onClick={() => setValueView("portfolio")}
                  className="text-xs h-7 px-2"
                >
                  Portfolio
                </Button>
                <Button
                  variant={valueView === "cost" ? "secondary" : "ghost"}
                  size="sm"
                  onClick={() => setValueView("cost")}
                  className="text-xs h-7 px-2"
                >
                  Cost Basis
                </Button>
              ...
              <div className="space-y-1">
                <p className="text-xs font-medium text-muted-foreground">
                  {valueView === "portfolio" ? "Portfolio Value" : "Total Cost Basis"}
                </p>
                <p className="text-2xl font-bold">
                  {valueView === "portfolio" ? formatCurrency(totalPortfolioValue) : formatCurrency(totalCost)}
                </p>
                <p className="text-xs text-muted-foreground">
                  {valueView === "portfolio" ? `Current holdings (${holdings.length} positions)` : ""}
                </p>
              </div>
            </CardContent>
          </Card>
              ...
              {holdings.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={8} className="text-center py-8 text-muted-foreground">
                    No holdings yet. Upload transactions to get started.
                  </TableCell>
                </TableRow>
              ) : (
                holdings.map((holding) => (
                  <TableRow key={`${holding.symbol}-${holding.broker}`}>
                    ...
                    <TableCell className="text-right font-semibold">{formatCurrency(holding.marketValue)}</TableCell>
                    <TableCell className="text-right">{holding.allocation.toFixed(1)}%</TableCell>
                    <TableCell className="text-right">
                      <div className={holding.todayGain >= 0 ? "text-green-600" : "text-red-600"}>
                        <div className="font-semibold">{formatCurrency(holding.todayGain)}</div>
                        <div className="text-xs">{formatPercent(holding.todayGainPercent)}</div>
                      </div>
                    </TableCell>
                    <TableCell className="text-right">
                      <div className={holding.totalGain >= 0 ? "text-green-600" : "text-red-600"}>
                        <div className="font-semibold">{formatCurrency(holding.totalGain)}</div>
