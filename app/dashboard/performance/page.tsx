...
const getCached = (): CachedPerformanceData | null => {
...
const setCache = (data: Omit<CachedPerformanceData, 'transactionCount'>): void => {
...
// Calculate real Beta (vs S&P 500)
const calculateBeta = (holdings: Holding[], benchmarkReturn: number): number => {
  ...
  const weightedPortfolioReturn = portfolioReturns.reduce(
...
// Calculate real Volatility (annualized standard deviation)
...
const calculateAttribution = (holdings: Holding[]) => {
  ...
  holdings.forEach(h => {
    ...
    bySector.set(sector, {
      return: current.return + stockReturn,
...
const calculateFactorExposure = (holdings: Holding[]) => {
  ...
  return {
    size: {
      largeCap: largeCapWeight * 100,
      midCap: midCapWeight * 100,
      smallCap: smallCapWeight * 100
    },
    style: {
      growth: growthWeight * 100,
      value: valueWeight * 100
    }
  }
}
...
export default function PerformancePage() {
  const { benchmarks, loading: contextLoading } = usePortfolio()
  ...
  const dividendData = useMemo(() => {
    ...
    const avgYield = totalValue > 0 
      ? dividendStocks.reduce((sum, s) => sum + (s.yield * s.marketValue / totalValue), 0)
      : 0
  ...
  const metrics = useMemo(() => {
    if (!holdings || holdings.length === 0) {
      return {
        ...
        var95: 0,
        attribution: [],
        factors: { size: { largeCap: 0, midCap: 0, smallCap: 0 }, style: { growth: 0, value: 0 } },
        periodReturns: {}
      }
    }
    ...
    const txnsThisYear = transactions.filter(t => new Date(t.date) >= yearStart)
    const costThisYear = txnsThisYear
      .filter(t => t.type === 'BUY')
  ...
  const riskGaugeData = [
    {
      name: 'Risk',
      value: Math.min(Math.abs(metrics.beta) * 50, 100),
      fill: metrics.beta > 1.5 ? '#ef4444' : metrics.beta > 1 ? '#f59e0b' : '#22c55e'
    }
  ]
    ...
    <div className="p-4 lg:p-6 space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {formatCurrency(metrics.totalGain)} gain
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {(100 - Math.min(Math.abs(metrics.beta) * 50, 100)).toFixed(0)}/100
            </div>
            <p className="text-sm text-muted-foreground mt-1">
      ...
      <Tabs value={selectedTab} onValueChange={setSelectedTab} className="space-y-4">
        ...
        <TabsContent value="overview" className="space-y-4">
          {/* Historical Chart with Benchmark Selector */}
          <Card>
            <CardHeader>
              ...
              <div className="h-[400px]">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={historicalData}>
                    ...
                    {/* Dynamic Benchmarks - Curved Lines */}
                    ...
                    {selectedBenchmarks.includes('russell2000') && (
                      <Line
                        type="monotoneX"
                        dataKey="russell2000"
                        stroke="#ec4899"
                    ...
                    {selectedBenchmarks.includes('vti') && (
                      <Line
                        type="monotoneX"
                        dataKey="vti"
                        stroke="#06b6d4"
                    ...
                    {selectedBenchmarks.includes('voo') && (
                      <Line
                        type="monotoneX"
                        dataKey="voo"
                        stroke="#84cc16"
                    ...
                    {selectedBenchmarks.includes('vxus') && (
                      <Line
                        type="monotoneX"
                        dataKey="vxus"
                        stroke="#f97316"
            ...
            <Card>
              ...
              <CardContent>
                <div className="h-[300px] flex items-center justify-center">
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      >
                        <tspan x="50%" dy="-10" fontSize="32" fontWeight="bold">
                          {(100 - riskGaugeData[0].value).toFixed(0)}
                        </tspan>
                        <tspan x="50%" dy="25" fontSize="14" className="fill-muted-foreground">
                          Risk Score
                        </tspan>
                      </text>
                    </RadialBarChart>
                ...
                <p className="text-sm text-center text-muted-foreground">
                  {metrics.beta < 0.8 ? 'Low Risk - Conservative' : 
                   metrics.beta < 1.2 ? 'Moderate Risk - Balanced' : 
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-3 rounded-lg bg-secondary/30">
                    <div>
                      <p className="text-sm text-muted-foreground">Beta (Market Sensitivity)</p>
                      <TooltipProvider>
                        <UITooltip>
                          <TooltipTrigger>
                          ...
                          <TooltipContent>
                            <p className="max-w-xs text-xs">1.0 = moves with market, &gt;1.0 = more volatile, &lt;1.0 = less volatile</p>
                          </TooltipContent>
                        </UITooltip>
                      </TooltipProvider>
                    </div>
                    ...
                    <div>
                      ...
                      <TooltipProvider>
                        <UITooltip>
                          ...
                          <TooltipContent>
                            <p className="max-w-xs text-xs">Risk-adjusted return. &gt;1.0 is good, &gt;2.0 is excellent</p>
                          </TooltipContent>
                        </UITooltip>
                      </TooltipProvider>
                    </div>
                    <p className={`text-xl font-bold ${metrics.sharpe > 1 ? 'text-green-600' : 'text-yellow-600'}`}>
                      {metrics.sharpe.toFixed(2)}
                    </p>
                  </div>
                    ...
                    <div>
                      ...
                      <TooltipProvider>
                        <UITooltip>
                          ...
                          <TooltipContent>
                            <p className="max-w-xs text-xs">Downside risk-adjusted return (better than Sharpe)</p>
                          </TooltipContent>
                        </UITooltip>
                      </TooltipProvider>
                    </div>
                    <p className="text-xl font-bold text-green-600">{metrics.sortino.toFixed(2)}</p>
                  </div>
                    ...
                    <div>
                      ...
                      <TooltipProvider>
                        <UITooltip>
                          ...
                          <TooltipContent>
                            <p className="max-w-xs text-xs">Annualized standard deviation of returns</p>
                          </TooltipContent>
                        </UITooltip>
                      </TooltipProvider>
                    </div>
                    ...
                    <div>
                      ...
                      <TooltipProvider>
                        <UITooltip>
                          ...
                          <TooltipContent>
                            <p className="max-w-xs text-xs">Largest peak-to-trough decline</p>
                          </TooltipContent>
                        </UITooltip>
                      </TooltipProvider>
                    </div>
                    <p className="text-xl font-bold text-red-600">{metrics.maxDrawdown.toFixed(2)}%</p>
                  </div>
                    ...
                    <div>
                      ...
                      <TooltipProvider>
                        <UITooltip>
                          ...
                          <TooltipContent>
                            <p className="max-w-xs text-xs">Maximum expected 1-day loss (95% confidence)</p>
                          </TooltipContent>
                        </UITooltip>
                      </TooltipProvider>
                    </div>
                    <p className="text-xl font-bold text-orange-600">{formatCurrency(metrics.var95)}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
                    ...
                    <BarChart data={metrics.attribution} layout="vertical">
                      ...
                      <Bar dataKey="contribution" name="Contribution to Return" radius={[0, 8, 8, 0]}>
                        {metrics.attribution.map((entry, index) => (
                          <Cell 
                            key={`cell-${index}`} 
                            fill={entry.contribution >= 0 ? '#22c55e' : '#eb4a14'} 
                          />
                        ))}
                      </Bar>
                    </BarChart>
                ...
                <div className="space-y-4">
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-sm font-bold">{metrics.factors.size.largeCap.toFixed(1)}%</span>
                    </div>
                    <div className="h-2 w-full bg-secondary rounded-full overflow-hidden">
                      <div 
                        className="h-full bg-blue-500"
                        style={{ width: `${metrics.factors.size.largeCap}%` }}
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-sm font-bold">{metrics.factors.size.midCap.toFixed(1)}%</span>
                    </div>
                    <div className="h-2 w-full bg-secondary rounded-full overflow-hidden">
                      <div 
                        className="h-full bg-green-500"
                        style={{ width: `${metrics.factors.size.midCap}%` }}
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-sm font-bold">{metrics.factors.size.smallCap.toFixed(1)}%</span>
                    </div>
                    <div className="h-2 w-full bg-secondary rounded-full overflow-hidden">
                      <div 
                        className="h-full bg-orange-500"
                        style={{ width: `${metrics.factors.size.smallCap}%` }}
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-sm font-bold">{metrics.factors.style.growth.toFixed(1)}%</span>
                    </div>
                    <div className="h-2 w-full bg-secondary rounded-full overflow-hidden">
                      <div 
                        className="h-full bg-purple-500"
                        style={{ width: `${metrics.factors.style.growth}%` }}
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-sm font-bold">{metrics.factors.style.value.toFixed(1)}%</span>
                    </div>
                    <div className="h-2 w-full bg-secondary rounded-full overflow-hidden">
                      <div 
                        className="h-full bg-teal-500"
                        style={{ width: `${metrics.factors.style.value}%` }}
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-6 p-4 bg-secondary/50 rounded-lg">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {metrics.factors.style.growth > 70 ? 
                      ...
                      "Strong value tilt - lower volatility, dividend focus, may underperform in bull markets" :
                      "Balanced growth/value mix - diversified style exposure"}
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <CardContent>
              {benchmarks && Object.keys(benchmarks).length > 0 ? (
                ...
                <div className="text-center py-12 text-muted-foreground">
                  ...
                  <p className="text-xs mt-2">Benchmark data is loading from your portfolio context</p>
                </div>
              )}
            </CardContent>
          </Card>
        ...
        <TabsContent value="dividends" className="space-y-4">
          <div className="grid md:grid-cols-3 gap-4">
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-green-600">
                  {formatCurrency(dividendData.totalAnnual)}
                </div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {dividendData.avgYield.toFixed(2)}%
                </div>
                <p className="text-xs text-muted-foreground mt-1">
                  Weighted average
                </p>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {formatCurrency(dividendData.totalMonthly)}/mo
                </div>
                <p className="text-xs text-muted-foreground mt-1">
                  Average monthly
                </p>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {dividendData.dividendStocks.length} stocks generating income
              </CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    <div key={stock.symbol} className="flex items-center justify-between p-4 rounded-lg bg-secondary/30 hover:bg-secondary/50 transition-colors">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-sm font-semibold text-green-600 mt-1">
                          {formatCurrency(stock.totalAnnual)}/yr
                        </p>
                        <p className="text-xs text-muted-foreground">
                          ({formatCurrency(stock.monthlyAvg)}/mo)
                        </p>
                      </div>
                    </div>
                  ))}
