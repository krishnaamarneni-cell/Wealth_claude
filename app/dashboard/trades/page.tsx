...
function InteractiveTradeDonut({ trades }: { trades: Trade[] }) {
  ...
  const prepareChartData = () => {
    ...
    if (Object.keys(tradesBySymbol).length === 0) return { stocks: [], total: 0 }

    const total = Object.values(tradesBySymbol).reduce((sum, val) => sum + val, 0)
    const stocks = Object.entries(tradesBySymbol)
      .map(([symbol, value], index) => ({ symbol, value, percentage: total ? (value / total) * 100 : 0, color: CHART_COLORS[index % CHART_COLORS.length] }))
  ...
  if (stocks.length === 0) {
    return (
      <Card className="border-border bg-card">
        <CardHeader><CardTitle className="text-base">Trade Volume by Symbol</CardTitle></CardHeader>
        <CardContent><div className="h-[400px] flex items-center justify-center text-muted-foreground">No trades found</div></CardContent>
      </Card>
    )
  }
          ...
          <div className="flex-1">
            <svg viewBox="0 0 400 400" className="w-full h-auto max-w-[400px] mx-auto">
              {segments.map((segment, index) => {
                ...
                return (
                  <path key={index} d={createArc(segment.startAngle, segment.endAngle, 100, outerRadius)} fill={segment.color} opacity={hoveredStock && !isHovered ? 0.3 : 1} className="transition-all duration-200 cursor-pointer" onMouseEnter={() => setHoveredStock(segment.symbol)} onMouseLeave={() => setHoveredStock(null)} />
                )
              })}
              {hoveredStock ? (
                <>
                  <text x="200" y="185" textAnchor="middle" fill="#ffffff" fontSize="14" fontWeight="500">{hoveredStock}</text>
                  <text x="200" y="210" textAnchor="middle" fill="#22c55e" fontSize="20" fontWeight="700">{formatCurrency(segments.find((s) => s.symbol === hoveredStock)?.value || 0)}</text>
                  <text x="200" y="230" textAnchor="middle" fill="#ffffff" fontSize="12">{segments.find((s) => s.symbol === hoveredStock)?.percentage.toFixed(1)}% of volume</text>
                </>
              ) : (
                <>
                  <text x="200" y="185" textAnchor="middle" fill="#ffffff" fontSize="14" fontWeight="500">Total Volume</text>
                  <text x="200" y="215" textAnchor="middle" fill="#22c55e" fontSize="24" fontWeight="700">{formatCurrency(total)}</text>
                </>
              )}
            </svg>
          </div>
          <div className="flex-1 max-w-md">
            <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2">
              {displayStocks.map((stock) => (
                <div key={stock.symbol} className={`flex items-center justify-between p-3 rounded-lg cursor-pointer transition-all duration-200 ${hoveredStock === stock.symbol ? "bg-gray-100/50 dark:bg-gray-800/50 ring-2 ring-gray-300 dark:ring-gray-700" : "hover:bg-gray-50 dark:hover:bg-gray-800/30"}`} onMouseEnter={() => setHoveredStock(stock.symbol)} onMouseLeave={() => setHoveredStock(null)}>
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs font-medium text-green-500">{formatCurrency(stock.value)}</div>
                  </div>
                </div>
              ))}
            </div>
...
export default function TradesPage() {
  ...
  const monthlyTrades = filteredTrades.reduce((acc, trade) => {
    ...
    if (!acc[month]) acc[month] = { buys: 0, sells: 0 }
    if (trade.type === "BUY") acc[month].buys += trade.total
    else acc[month].sells += trade.total
    return acc
  }, {} as Record<string, { buys: number; sells: number }>)

  const chartData = Object.entries(monthlyTrades)
    .map(([month, data]) => ({ month, buys: data.buys, sells: data.sells }))
        ...
        <>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Card className="border-border bg-card">
              ...
              <CardContent>
                <div className="text-2xl font-bold text-foreground">{filteredTrades.length}</div>
                <p className="text-xs text-muted-foreground">{buyTrades.length} buys, {sellTrades.length} sells</p>
              </CardContent>
            </Card>
            <Card className="border-border bg-card">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
              ...
              <CardContent>
                <div className="text-2xl font-bold text-primary">{formatCurrency(totalBuyVolume)}</div>
            ...
            <Card className="border-border bg-card">
              ...
              <CardContent>
                <div className="text-2xl font-bold text-destructive">{formatCurrency(totalSellVolume)}</div>
                ...
                <div className="h-[300px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={chartData}>
                      ...
                      <YAxis tickFormatter={(v) => `$${(v / 1000).toFixed(0)}k`} stroke="#ffffff" fontSize={12} tick={{ fill: '#ffffff' }} />
                      <Tooltip cursor={{ fill: 'transparent' }} contentStyle={{ backgroundColor: "hsl(var(--popover))", border: "1px solid hsl(var(--border))", borderRadius: "8px", color: "hsl(var(--popover-foreground))" }} formatter={(value: number) => [formatCurrency(value), ""]} />
                      <Legend />
          ...
          <Card className="border-border bg-card">
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <Select value={String(rowsPerPage)} onValueChange={handleRowsPerPageChange}>
                  <SelectTrigger className="w-[70px] h-8 text-foreground"><SelectValue /></SelectTrigger>
                  <SelectContent>
            ...
            <CardContent>
                  ...
                  <TableBody>
                    {paginatedTrades.map((trade, index) => (
                      <TableRow key={`${trade.date}-${trade.symbol}-${startIndex + index}`}>
                        ...
                        <TableCell><Badge variant={trade.type === "BUY" ? "default" : "destructive"}>{trade.type}</Badge></TableCell>
                        ...
                        <TableCell className="text-right text-muted-foreground">{formatCurrency(trade.fees ?? 0)}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
              ...
              <div className="flex items-center justify-between border-t border-border pt-4 mt-4">
                <p className="text-sm text-muted-foreground">Showing {startIndex + 1}–{Math.min(endIndex, filteredTrades.length)} of {filteredTrades.length} trades</p>
                <div className="flex items-center gap-2">
