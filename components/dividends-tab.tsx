...
import {
  TrendingUp, Calendar, RefreshCw, Clock, Search,
  ArrowUpDown, DollarSign, AlertCircle, ChevronDown, ChevronUp,
} from "lucide-react"
...
interface MonthlyDividendData {
  [symbol: string]: { [month: string]: number }
}
...
const CHART_COLORS = ["#3b82f6", "#8b5cf6", PRIMARY_GREEN, "#f59e0b", "#ef4444", "#06b6d4", "#ec4899", "#14b8a6"]
...
const CylinderBar: React.FC<any> = ({ x, y, width, height, fill, stroke, strokeWidth = 0 }) => {
  ...
  return (
    <g>
      <rect x={x} y={topY} width={width} height={height} fill={fill} stroke={stroke} strokeWidth={strokeWidth} rx={radius} ry={radius} />
      <ellipse cx={centerX} cy={topY} rx={radius} ry={radius / 2} fill={fill} stroke={stroke} strokeWidth={strokeWidth} />
    </g>
  )
}
...
function InteractiveDividendDonut() {
  ...
  const prepareChartData = () => {
    ...
    if (dividendStocks.length === 0) return { stocks: [], total: 0, totalDividends: 0 }
    const total = dividendStocks.reduce((sum, h) => sum + (viewType === "current" ? h.marketValue : h.totalCost) || 0, 0)
    ...
    const stocks = dividendStocks
      .map((h, index) => {
        ...
        return { symbol: h.symbol, value, percentage: total ? (value / total) * 100 : 0, dividendAmount, color: CHART_COLORS[index % CHART_COLORS.length] }
      })
  ...
  const segments = stocks.map((stock) => {
    ...
    const segment = { ...stock, startAngle: currentAngle, endAngle: currentAngle + angle }
    currentAngle += angle
    return segment
  })
  ...
  if (stocks.length === 0) {
    return (
      <Card className="border-border bg-card">
        <CardHeader><CardTitle className="text-base">Dividend Portfolio Distribution</CardTitle></CardHeader>
        <CardContent><div className="h-[400px] flex items-center justify-center text-muted-foreground">No dividend-paying stocks found</div></CardContent>
      </Card>
    )
  }
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Based on {viewType === "current" ? "current market value" : "invested amount"}</p>
          </div>
          <div className="flex gap-2">
            <button onClick={() => setViewType("current")} className={`px-3 py-1 text-sm rounded-md transition-colors ${viewType === "current" ? "bg-green-500 text-white" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"}`}>Current</button>
            <button onClick={() => setViewType("invested")} className={`px-3 py-1 text-sm rounded-md transition-colors ${viewType === "invested" ? "bg-green-500 text-white" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"}`}>Invested</button>
          </div>
        </div>
      </CardHeader>
      <CardContent>
        <div className="flex flex-col lg:flex-row gap-6">
          <div className="flex-1">
            <svg viewBox="0 0 400 400" className="w-full h-auto max-w-[400px] mx-auto">
              ...
              {hoveredStock ? (
                <>
                  <text x="200" y="185" textAnchor="middle" fill="#ffffff" fontSize="14" fontWeight="500">{hoveredStock}</text>
                  <text x="200" y="210" textAnchor="middle" fill="#22c55e" fontSize="20" fontWeight="700">{formatCurrency(segments.find((s) => s.symbol === hoveredStock)?.dividendAmount || 0)}</text>
                  <text x="200" y="230" textAnchor="middle" fill="#ffffff" fontSize="12">{segments.find((s) => s.symbol === hoveredStock)?.percentage.toFixed(1)}% of portfolio</text>
                </>
              ) : (
                <>
                  <text x="200" y="185" textAnchor="middle" fill="#ffffff" fontSize="14" fontWeight="500">Total Dividends</text>
                  <text x="200" y="215" textAnchor="middle" fill="#22c55e" fontSize="24" fontWeight="700">{formatCurrency(totalDividends)}</text>
                </>
              )}
            </svg>
          </div>
          <div className="flex-1 max-w-md">
            <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2">
              {displayStocks.map((stock) => (
                <div
                  key={stock.symbol}
                  className={`flex items-center justify-between p-3 rounded-lg cursor-pointer transition-all duration-200 ${hoveredStock === stock.symbol ? "bg-gray-100/50 dark:bg-gray-800/50 ring-2 ring-gray-300 dark:ring-gray-700" : "hover:bg-gray-50 dark:hover:bg-gray-800/30"}`}
                ...
                >
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-green-600 font-medium">{formatCurrency(stock.dividendAmount)}</div>
                  </div>
                </div>
              ))}
            </div>
...
function CalendarSection() {
  ...
  const [sortConfig, setSortConfig] = useState<{ key: keyof DividendTransactionRow; direction: "asc" | "desc" } | null>(null)
  ...
  useEffect(() => {
    const fetchUpcomingDividends = async () => {
      ...
      try {
        ...
        const now = new Date()
        const events: DividendCalendarEvent[] = (data as any[])
          .map((d) => ({
            symbol: d.symbol,
            exDate: d.exDate,
            payDate: d.payDate,
            ...
            daysUntil: Math.ceil((new Date(d.payDate).getTime() - now.getTime()) / (1000 * 60 * 60 * 24)),
          }))
  ...
  const generateMonthlyCalendar = () => {
    ...
    const sourceDividendsByMonth = Object.keys(income.dividendsByMonth).length > 0 ? income.dividendsByMonth : income.dividendsByMonth
  ...
  const dividendTransactions: DividendTransactionRow[] = useMemo(() => {
    return transactions
      ...
      .map((t) => ({
        ...
        divPerShare: t.price ? Math.abs(parseFloat(String(t.price)) || 0) : 0,
      }))
  ...
  const filteredAndSortedTransactions = useMemo(() => {
    ...
    if (searchTicker) filtered = filtered.filter((t) => t.symbol.toLowerCase().includes(searchTicker.toLowerCase()))
    if (sortConfig) {
  ...
  const SortIcon = ({ columnKey }: { columnKey: keyof DividendTransactionRow }) => {
    if (!sortConfig || sortConfig.key !== columnKey) return <ArrowUpDown className="h-3 w-3 ml-1 inline opacity-30" />
    return <ArrowUpDown className={`h-3 w-3 ml-1 inline ${sortConfig.direction === "desc" ? "rotate-180" : ""}`} />
  }
  ...
  return (
    <Card className="border-border bg-card">
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="text-base"><Calendar className="inline h-4 w-4 mr-2" />Dividend Calendar</CardTitle>
          <div className="flex gap-2">
            <button onClick={() => setCalendarView("upcoming")} className={`px-3 py-1 text-sm rounded-md transition-colors ${calendarView === "upcoming" ? "bg-green-500 text-white" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"}`}>Upcoming</button>
            <button onClick={() => setCalendarView("historical")} className={`px-3 py-1 text-sm rounded-md transition-colors ${calendarView === "historical" ? "bg-green-500 text-white" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"}`}>Historical</button>
            <button onClick={() => setCalendarView("transactions")} className={`px-3 py-1 text-sm rounded-md transition-colors ${calendarView === "transactions" ? "bg-green-500 text-white" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"}`}>Transactions</button>
          </div>
        </div>
      </CardHeader>
      <CardContent>
              ...
              <TableBody>
                {upcomingDividends.slice(0, 20).map((event, idx) => (
                  <TableRow key={idx}>
                    ...
                    <TableCell className="text-right font-medium text-green-600">{formatCurrency(event.amount)}</TableCell>
                    <TableCell className="text-right">
                      <span className={`px-2 py-1 rounded-md text-xs font-medium ${event.daysUntil < 7 ? "bg-orange-500/10 text-orange-500" : "bg-blue-500/10 text-blue-500"}`}>
                        {event.daysUntil}d
                      </span>
                    </TableCell>
                  </TableRow>
            ...
            <Table>
              ...
              <TableBody>
                {Object.entries(monthlyCalendar)
                  .sort(([, a], [, b]) => {
                    const aTotal = Object.values(a).reduce((sum: number, val: number) => sum + val, 0)
                    const bTotal = Object.values(b).reduce((sum: number, val: number) => sum + val, 0)
                    return bTotal - aTotal
                  })
                  .map(([symbol, months]) => (
                    <TableRow key={symbol}>
                      <TableCell className="sticky left-0 bg-card z-10 font-medium">{symbol}</TableCell>
                      {calendarMonths.slice(-18, -6).map((month) => (
          ...
          <div>
            ...
            <div className="overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="cursor-pointer hover:bg-muted/50" onClick={() => handleSort("date")}>Date <SortIcon columnKey="date" /></TableHead>
                    <TableHead className="cursor-pointer hover:bg-muted/50" onClick={() => handleSort("symbol")}>Ticker <SortIcon columnKey="symbol" /></TableHead>
                    <TableHead className="text-right cursor-pointer hover:bg-muted/50" onClick={() => handleSort("dividendAmount")}>Dividend Amount <SortIcon columnKey="dividendAmount" /></TableHead>
                    <TableHead className="text-right cursor-pointer hover:bg-muted/50" onClick={() => handleSort("shares")}>Shares Eligible <SortIcon columnKey="shares" /></TableHead>
                    <TableHead className="text-right cursor-pointer hover:bg-muted/50" onClick={() => handleSort("divPerShare")}>Div Per Share <SortIcon columnKey="divPerShare" /></TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {filteredAndSortedTransactions.length > 0 ? (
                    filteredAndSortedTransactions.map((txn, idx) => (
                      <TableRow key={idx}>
                        ...
                        <TableCell className="text-right font-medium text-green-600">{formatCurrency(txn.dividendAmount)}</TableCell>
                        <TableCell className="text-right">{txn.shares.toFixed(4)}</TableCell>
                        <TableCell className="text-right">{formatCurrency(txn.divPerShare)}</TableCell>
                      </TableRow>
                    ))
                  ) : (
...
const FORECAST_CACHE_KEY = "dividendPageForecasts_session"
const FORECAST_LOADED_KEY = "dividendPageLoaded_session"
...
export default function DividendsTab() {
  ...
  const { income, holdings, portfolioValue, transactions, isLoading } = usePortfolio()
  ...
  useEffect(() => {
    ...
    const build = async () => {
      try {
        ...
        sessionStorage.setItem("calculatedDividendsFromTxn", JSON.stringify(result))
      } catch (err) {
    ...
    const loadDividendData = async () => {
      ...
      try {
        ...
        if (cached && cachedLoaded === "true") {
          ...
          const calcRaw = sessionStorage.getItem("calculatedDividendsFromTxn")
          let calculated: DividendForecast[] = []
          if (calcRaw) calculated = JSON.parse(calcRaw)
          const merged = { ...parsed, ...calculated }
          setForecasts(merged as any)
  ...
  const ytdPaymentCount = receivedDividends.filter((f) => new Date(f.date) >= startOfYear).length

  const startOfLastYear = new Date(now.getFullYear() - 1, 0, 1)
  const endOfLastYear = new Date(now.getFullYear() - 1, 11, 31)
  const lastYearDividends = receivedDividends
    .filter((f) => { const date = new Date(f.date); return date >= startOfLastYear && date <= endOfLastYear })
    .reduce((sum, f) => sum + f.totalAmount, 0)

  const yoyGrowthRate = lastYearDividends > 0 ? ((ytdDividends - lastYearDividends) / lastYearDividends) * 100 : 0
  ...
  const upcoming12Months = upcomingDividends
    ...
    .reduce((sum, f) => sum + f.totalAmount, 0)
  const upcoming12MonthsCount = upcomingDividends.filter((f) => new Date(f.date) <= oneYearFromNow).length
  ...
  // Monthly chart data
  const monthlyRaw = forecasts.reduce((acc, f) => {
    const month = f.date.substring(0, 7)
    const existing = acc.find((d) => d.month === month)
    if (existing) {
      if (f.type === "received") existing.received += f.totalAmount
      else existing.upcoming += f.totalAmount
    } else {
      acc.push({ month, received: f.type === "received" ? f.totalAmount : 0, upcoming: f.type === "upcoming" ? f.totalAmount : 0 })
    }
    return acc
  }, [] as { month: string; received: number; upcoming: number }[])
  monthlyRaw.sort((a, b) => a.month.localeCompare(b.month))
  const monthlyChartData = monthlyRaw.map((m) => ({ month: m.month, received: m.received, upcoming: m.upcoming, total: m.received + m.upcoming }))

  // Top payers
  const dividendBySymbol = forecasts.reduce((acc, f) => {
    if (!acc[f.symbol]) acc[f.symbol] = { symbol: f.symbol, total: 0, count: 0 }
    acc[f.symbol].total += f.totalAmount
    acc[f.symbol].count++
    return acc
  }, {} as Record<string, { symbol: string; total: number; count: number }>)
  const topPayers = Object.values(dividendBySymbol).sort((a, b) => b.total - a.total).slice(0, 5)
  ...
  const avgMonthlyIncome = ytdDividends / (now.getMonth() + 1)
  ...
  useEffect(() => {
    ...
    dividends.forEach((div) => {
      ...
      const matchingBuys = transactions.filter((t) => {
        ...
        const daysDiff = (buyDate.getTime() - divDate.getTime()) / (1000 * 60 * 60 * 24)
      ...
      if (matchingBuy) {
        ...
        drips.push({ date: matchingBuy.date, symbol: matchingBuy.symbol, shares, price: parseFloat(String(matchingBuy.price)) || 0, amount: buyAmount })
      }
    })
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="pt-4 pb-4">
          <div className="flex items-center gap-2">
            <Button variant={dividendSubTab === "overview" ? "default" : "outline"} size="sm" onClick={() => setDividendSubTab("overview")} className="h-9">Overview</Button>
            <Button variant={dividendSubTab === "growth" ? "default" : "outline"} size="sm" onClick={() => setDividendSubTab("growth")} className="h-9">Growth & DRIP</Button>
          </div>
        </CardContent>
      </Card>
      ...
      {dividendSubTab === "overview" && (
        <>
            ...
            <Card>
              <CardContent className="pt-4 pb-4">
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground">Number of Assets: {totalStocks}</p>
                  <p className="text-xs text-muted-foreground">Dividend-Paying: {dividendPayingStocks}</p>
                </div>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={400}>
                  <BarChart data={monthlyChartData.slice(-12)}>
                    ...
                    <XAxis dataKey="month" tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 12 }} tickFormatter={(value) => { const date = new Date(value + "-01"); return date.toLocaleDateString("en-US", { month: "short" }) }} />
                    ...
                    <Legend />
                    <Bar dataKey="received" fill={PRIMARY_GREEN} name="Received" shape={<CylinderBar />} />
                    <Bar dataKey="upcoming" fill="#3b82f6" name="Upcoming" shape={<CylinderBar />} />
                  </BarChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base"><TrendingUp className="inline h-4 w-4 mr-2" />Top 5 Dividend Payers</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-2">
                {topPayers.length > 0 ? topPayers.map((payer, idx) => (
                  <div key={payer.symbol} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                    <div className="flex items-center gap-3">
      ...
      {dividendSubTab === "growth" && (
        <>
            ...
            <Card>
              <CardContent className="pt-4 pb-4">
                <div className="space-y-1">
                  ...
                  <p className={`text-2xl font-bold ${yoyGrowthRate >= 0 ? "text-green-600" : "text-red-600"}`}>{yoyGrowthRate >= 0 ? "+" : ""}{yoyGrowthRate.toFixed(2)}%</p>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base"><TrendingUp className="inline h-4 w-4 mr-2" />Dividend Growth Over Time</CardTitle>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={400}>
                <LineChart data={growthChartData}>
                  <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                  <XAxis dataKey="month" tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 12 }} tickFormatter={(value) => { const date = new Date(value + "-01"); return date.toLocaleDateString("en-US", { month: "short", year: "2-digit" }) }} />
                  <YAxis tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 12 }} tickFormatter={(value) => `$${value}`} />
                  <Tooltip formatter={(value: number) => formatCurrency(value)} contentStyle={{ backgroundColor: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: "8px" }} />
                  <Legend />
                  <Line type="monotone" dataKey="total" stroke={PRIMARY_GREEN} strokeWidth={3} name="Total Dividends" dot={{ r: 4 }} />
                </LineChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base"><DollarSign className="inline h-4 w-4 mr-2" />Dividend Reinvestment (DRIP)</CardTitle>
            </CardHeader>
            <CardContent>
              {dripTransactions.length > 0 ? (
                  ...
                  <TableBody>
                    {dripTransactions.map((drip, idx) => (
                      <TableRow key={idx}>
                        ...
                        <TableCell className="text-right font-medium text-green-600">{formatCurrency(drip.amount)}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
                ...
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm mt-1">DRIP transactions are detected when a BUY matches a DIVIDEND within 3 days</p>
                </div>
              )}
            </CardContent>
          </Card>
